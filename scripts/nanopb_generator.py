    ...
    def __str__(self):
        ...
        if self.allocation == 'POINTER':
            if self.rules == 'REPEATED':
                ...
                result += '    uint32_t ' + var_name + '_count;\n'
...
class OneOf(Field):
    ...
    def __str__(self):
        ...
        if self.fields:
            ...
            result += '    uint32_t which_' + Globals.naming_style.var_name(self.name) + ";\n"
            if self.anonymous:
                result += '    union {\n'
            else:
...
class Message(ProtoElement):
    ...
    def fields_declaration_cpp_lookup(self, local_defines):
        ...
        result += '    static PB_INLINE_CONSTEXPR const uint32_t fields_array_length = %d;\n' % (self.count_all_fields())

        size_define = "%s_size" % (self.name)
        if size_define in local_defines:
            result += '    static PB_INLINE_CONSTEXPR const uint32_t size = %s;\n' % (size_define)

        result += '    static inline const pbjson_msgdesc_t* fields() {\n'
        result += '        return &%s_msg;\n' % (self.name)
        result += '    }\n'
...
class ProtoFile:
    ...
    def generate_header(self, includes, headername, options):
        ...
        yield '#ifndef JSON_PB_%s_INCLUDED\n' % symbol
        yield '#define JSON_PB_%s_INCLUDED\n' % symbol
        if self.math_include_required:
            yield '#include <math.h>\n'
        try:
            yield options.libformat % ('pb/json.h')
        except TypeError:
        ...
        yield '#if PB_JSON_PROTO_HEADER_VERSION != 40\n'
        ...
        if self.messages:
            ...
            for msg in self.messages:
                yield 'extern const pbjson_msgdesc_t %s_msg;\n' % Globals.naming_style.type_name(msg.name)
            yield '\n'
    ...
    def generate_source(self, headername, options):
        ...
        # Check if any messages exceed the 64 kB limit of 16-bit uint32_t
        exceeds_64kB = []
        for msg in self.messages:
        ...
        # Generate the message field definitions (PBJSON_BIND() call)
        for msg in self.messages:
